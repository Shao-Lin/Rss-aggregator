const viewModal = (state, btn) => {
  ...
  descriptionText.textContent = `${state.feedsAndPosts.posts[btn.getAttribute('data-btn-id') - 1].description}`;
  title.textContent = `${state.feedsAndPosts.posts[btn.getAttribute('data-btn-id') - 1].title}`;
};
